...
const Manage_Project_1 = ({ setLoginUser }) => {
    ...
    const [_err, setErrorFlag] = useState({
        __err: ""
    })
    ...
    useEffect(() => {
        axios.post("http://localhost:1337/api/get_standard_json", { type: String(1 + step_no) })
            ...
            .catch(function (error) {
                // handle error
                console.log(error);
                alert("Error while fetching Standard JSON");
            })
            .then(function () {
                // always executed
            });
    ...
    const move_to_next_step = () => {
        navigate("../manage_project_2?proj_id=" + project_id + "&&p_name=" + project_name);
    }
                    ...
                    <tr>
                        <td id="left_panel">
                            ...
                            <p id="json_type_prop_display">
                                <span className="glyphicon glyphicon-hand-right"></span>&nbsp;<b>Scene JSON Properties</b><hr />
                                <div id="json_props">
                            ...
                            <table id="t1">
                                <tbody>
                                    ...
                                    <tr style={{ backgroundColor: "white", borderTop: "1px solid lightgray" }}>
                                        <td colSpan={2}>
                                            {/* <button disabled={_err.__err == "1" ? false : true} id="validate_btn" className='action_btn btn btn-primary btn-sm' onClick={validate_json_against_standard_json}>
                                                <span className='glyphicon glyphicon-check'></span>&nbsp;Validate
                                            </button>&nbsp;&nbsp;
                                            <button disabled={_err.__err == "-1" ? false : true} id="upload_btn" className='action_btn btn btn-primary btn-sm' onClick={upload}>
                                                <span className='glyphicon glyphicon-save'></span>&nbsp;Upload
                                            </button>&nbsp;&nbsp;
                                            <button disabled={_err.__err == "2" ? false : true} id="next_btn" className='action_btn btn btn-primary btn-sm' onClick={move_to_next_step}>
...
export default Manage_Project_1