...
const Manage_Project_4 = ({ setLoginUser }) => {
    ...
    const fetch_base64_strings = () => {
        axios.post("http://localhost:1337/api/fetch_encoded_strings", { proj_id: project_name })
            .then((response) => {
                let str_to_print = "";
                let base64_array = response.data.base64_strings;

                base64_array.map((item) => {
                    console.log(item.base_encoded_string);
                    console.log(item.base_encoded_string.length);
                    for (let i = 0; i < item.base_encoded_string.length; ++i){
                        str_to_print += "<p class='base64_strs'>" + item.base_encoded_string[i] + "</p><hr/>";
                    }
            })

        if (str_to_print.length == 0) {
            console.log("Empty String");
            return;
        }

        document.getElementById("base_encoded_str").innerHTML = str_to_print;

        // console.log(response.data.base64_strings.base_encoded_string);
        // console.log(response.data.message);
        // console.log(response.data.status);
        return;
    });
    ...
    if (window.confirm("Are you sure you want to upload ?")) {
        axios.post("http://localhost:1337/api/upload_json", input_json_validation)
            .then((response) => {
                ...
                document.getElementById("error_msg").innerHTML = "!!..JSON successfully uploaded..!!";
...
const move_to_next_step = () => {
    navigate("../manage_project_5?proj_id=" + project_id + "&&p_name=" + project_name);
}
...
return (
    <div>
        ...
        <table id="data_table" style={{ width: "90%" }}>
            <tbody>
                <tr>
                    <td id="left_panel">
                        <p id="home_btn">
                        ...
                        <p id="json_type_prop_display">
                            <span className="glyphicon glyphicon-hand-right"></span>&nbsp;<b>Custom JSON</b><hr />
                            <div id="json_props">
                    ...
                    <td id="mid_panel">
                        ...
                        <table id="t1">
                            <tbody>
                                <tr>
                                    <td style={{ borderRight: "1px solid lightgray", width: "30%", paddingTop: 2, paddingBottom: 0, paddingRight: 2, paddingLeft: 1 }}>
                                        <textarea rows="28" cols="38" id="json_copy_paste_textarea" name="input_json_content" onChange={handle_change} value={input_json_validation.input_json_content} placeholder="Copy and Paste JSON here"></textarea>
                                    </td>
                                    <td style={{ borderRight: "1px solid lightgray", verticalAlign: "top", width: "38%" }}>
                                        Base Encoded Strings
                                        <div id="base_encoded_str">
                                            <img src={LOADER} width="100px" />
                                        </div>
                                    </td>
                                    <td id="json_validation_msg_td" style={{ padding: 2 }}>
                                ...
                                <tr style={{ backgroundColor: "white", borderTop: "1px solid lightgray" }}>
                                    <td colSpan={3}>
                                        {/* <button disabled={err_flag == 1 ? false : true} id="validate_btn" className='action_btn btn btn-primary btn-sm' onClick={validate_json_against_standard_json}>
                                            ...
                                            <button disabled={err_flag == 2 ? false : true} id="next_btn" className='action_btn btn btn-primary btn-sm' onClick={move_to_next_step}>
                                        ...
                                        <button id="next_btn" className='action_btn btn btn-primary btn-sm' onClick={move_to_next_step}>
                                            <span className='glyphicon glyphicon-forward'></span>&nbsp;Next
                                        </button>
                                    </td>
                                </tr>
...
export default Manage_Project_4